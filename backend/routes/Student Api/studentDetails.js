...
router.get("/getAllDetails", async (req, res) => {
  try {
    ...
    students = await Promise.all(students.map(async (student) => {
      const { lastLogin, ipAddress } = student.credential || {};
      const userSession = await Session.findOne({ userId: student.credential, isLoggedIn: true });
      const isActive = userSession ? userSession.isLoggedIn : false;
